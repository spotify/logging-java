...
public class LoggerThresholdFilterTest {
    ...
    private List<String> variousLoggers = asList(spotifyLog,
            appLog,
            appPkgLog,
            appClassLog,
            orgLog,
    ...
    public LoggerThresholdFilterTest() {
        spotifyLoggers = new HashSet<String>(asList(spotifyLog, appLog, appPkgLog, appClassLog, spotifyLibraryLog));
    ...
    public FilterReply andFilterReplies(FilterReply first, FilterReply second) {
        if(first == FilterReply.DENY || second == FilterReply.DENY)
